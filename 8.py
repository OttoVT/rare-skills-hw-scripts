...
powers_of_T_tau1 = [multiply(G1, int(GF((tau ** i) * t(tau) / delta)) 
) for i in range(len(R))]
...
# Compute witness
out = GF(3) * x * x * y + GF(5) * x * y - x - GF(2)  *y + GF(3)# the witness vector with the intermediate variables inside
v1 = GF(3)*x*x
v2 = v1 * y
w = GF(np.array([1, out, x, y, v1, v2]))

# pick r & s and destroyed it after that
...
# c1Shifted = cShiftedprime1 + HT1 + a1Shifted * s + b1Shifted * r + delta1 * r * s
